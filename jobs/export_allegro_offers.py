...
Usage:
    ...
    python jobs/export_allegro_offers.py --input data/allegro_offers_2484-4979.csv
...
def _load_csv(path: Path) -> tuple[list[str], dict[str, dict[str, dict]]]:
    """Return (ordered_eans, data).
...
def main(input_path: str, output_path: str | None = None) -> None:
    ...
    ean_order, data = _load_csv(path)

    # Collect all sellers sorted alphabetically
    sellers: list[str] = sorted({s for ean_data in data.values() for s in ean_data})

    print(f"  {len(ean_order)} EANs  ·  {len(sellers)} sellers: {', '.join(sellers)}")
    ...
    wb = build_workbook(ean_order, data, sellers, path.name)
    wb.save(output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", default="data/allegro_offers_2484-4979.csv")
